...
\subsection{Example 1 -- network structure}

A set of example parameter files (\verb=param.example_0_0_j=) are provided in the \verb=param_files/= \verb=structure_example= folder. Each parameter file will run a dynamic network simulation with 250 edges for $2\text{x}10^6$ simulation time steps and a different value of the fusion rate. To run the examples, navigate to the \verb=param_files/structure_example= directory and run each example with
\begin{verbatim}
../../dynnetwork.exe example_0_0_j > stdout.example_0_0_j
\end{verbatim}

If your machine has multiple processors, the simulations can be run in parallel. Snapshots of the network structure will be written to the file (\verb=example_0_0_j.snap.out=) every 1000 simulation steps. Each fusion and fission event will be recorded in the file (\verb=example_0_0_j.ffevents.out=). Use the script \verb=analyze_structure_example.m= (in the scripts folder) to visualize a given network structure over time, and the resulting plot of $\hat{\rho}C_2$ once all 10 simulations are finished. To run this script successfully, you must also add \verb=github.com/lenafabr/networktools/NetworkObj.m= to your path. The resulting visualizations should look something like this:

\centerline{\includegraphics[width=0.9\textwidth]{structure_example_panel.pdf}}


\subsection{Example 2 -- transport on a dynamic network}

A second set of example parameter files (\verb=param.constconc_0_0_j=) are provided in the \verb=param_files/= \verb=transport_example= folder. Each parameter file will run a dynamic network simulation with 250 edges for $3\text{x}10^6-18\text{x}10^6$ simulation time steps and a different value of the decay rate. To run the examples, navigate to the \verb=param_files/transport_example= directory and run each example with
\begin{verbatim}
../../dynnetwork.exe constconc_0_0_j > stdout.constconc_0_0_j
\end{verbatim}

If your machine has multiple processors, the simulations can be run in parallel. Snapshots of the network structure will be written to the file (\verb=constconc_0_0_j.snap.out=). Each fusion and fission event will be recorded in the file (\verb=constconc_0_0_j.ffevents.out=). Use the script \verb=analyze_transport_example.m= (in the scripts folder) to visualize the spread of material over time and the eventual plot of $S$ (steady-state content) vs decay rate once all simulations are finished. To run this script successfully, you must also add \verb=github.com/lenafabr/= \verb=networktools/NetworkObj.m= to your path. The resulting visualizations should look something like this:

\centerline{\includegraphics[width=0.9\textwidth]{transport_example_panel.pdf}}
...
\item {\it SNAPSHOTFILE}
  \begin{itemize}
    ...
    \item File for dumping out snapshots. 
...
\item {\it REMODELINGFILE}
  ...
  \end{itemize}
%
\item {\it APPENDSNAPSHOTS}
  \begin{itemize}
    \item value: 1 logical; default: false
    \item Whether to append rather than rewriting the snapshot file
  \end{itemize}
%
\item {\it STARTSNAPSTEP}
  ...
  \end{itemize}
%
\item {\it SNAPSHOTEVERY}
  \begin{itemize}
    \item  value: 1 integer; default: 1
    \item Interval at which snapshots should be saved (in units of simulation steps).
  \end{itemize}
...
\item {\it DIFF}
  \begin{itemize}
    \item  value: 1 or NSPECIES floats ; default: 0.0
...
\item {\it NSUBSTEPS}
  \begin{itemize}
    \item  value: 1 integer ; default: 0
    \item The number of steps which should be taken during the diffusion calculation for each timestep used by the rest of the simulation. (i.e. value 10 means a timestep of DELT/10 will be used to perform the diffusion calculation). It is best to keep DIFF$\cdot$(DELT/NSUBSTEPS)/MITOLEN$^2$ at a value much smaller than 1. For default values of DELT and MITOLEN, this means DIFF/NSUBSTEPS should not exceed $\sim100$.
  \end{itemize}
%
\item {\it PRODRATE}
  \begin{itemize}
...
\item {\it NUMTRIGGERUNITS}
  \begin{itemize}
    \item  value: 1 integer ; default: $0$
    \item The number of edges at which diffusing particles should be produced or initialized for each species.
  \end{itemize}
%
...
\item {\it SEEDCONCS}
  \begin{itemize}
    \item  value: 1 logical; default false
    \item Whether to initially set the trigger units to concentration 1. This can be used with FIXEDGES set to false to simulate the dilution of some initial quantity of material.
  \end{itemize}
