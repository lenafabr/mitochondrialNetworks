...
CONTAINS
  SUBROUTINE RUNBROWNDYNSIM(NETP, NSTEP, DELT, DP, STARTTIME, CLOCKRATE)
    ...
    ! set the producer/fixed/initially-nonzero conc edges
    IF(NETP%FIXEDGES.OR.SEEDCONCS.OR.ANY(NETP%PRODRATE.GT.0D0)) THEN
            ...
            DO PC = 1,NETP%NUMTRIGGERUNITS 
               ...
               DO WHILE (.NOT.FOUND)
                  ...
                  IF(NETP%EDGEACT(EC).AND.ALL(SPECIESPRODUCER(CC,:).NE.EC)) THEN
                     ...
                     IF(NETP%FIXEDGES.OR.SEEDCONCS) NETP%CONC(EC,CC) = 1D0
                     FOUND = .TRUE.
  ...
  SUBROUTINE SETDYNPARAM(DP)
    ...
    USE KEYS, ONLY : OUTFILE, SNAPSHOTFILE, PRINTEVERY, SNAPSHOTEVERY, &
         &    APPENDSNAPSHOTS, DOBROWN, REMODELINGFILE, STARTSNAPSTEP,STARTDIFFSTEP, NSUBSTEPS
